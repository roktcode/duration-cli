...
async function readAPIKey() {
	...
	try {
		...
		throw new Error("Please first run 'dur init' command, then enter a valid Google API key!");
	}

	const { apiKey } = JSON.parse(data.toString());

	if (!apiKey) {
		throw new Error("Please enter GOOGLE API KEY by running `dur init` command");
	}
