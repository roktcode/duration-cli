...
export default function (durationData, files, failedFiles, parent) {
	...
	console.log(
		`${chalk.dim(` DIRECTORY: `)}${chalk.italic.yellow(
			process.cwd()
		)} ${chalk.dim(
			parent ? `(without sub-directories)` : `(and sub-directories)`
		)}`
	);
	...
	console.log(
		`${chalk.dim(` MEDIA FILES: `)}${chalk.cyan(
			files.length
		)} ${chalk.dim(`files`)}`
	);
	...
	console.log(
		`${chalk.dim(` CALCULATED: `)}${chalk.green(
			durationData.succeeded
		)} `
	);

	failedFiles.length > 0
		? console.log(`${chalk.dim(` ERRORS: `)}${chalk.red(durationData.failed)} `)
